...
class JudgeStatus extends React.Component<JudgeStatusProps, JudgeStatusState> {
  private timer?: ReturnType<typeof setInterval>;

  constructor(props: JudgeStatusProps) {
    super(props);
    this.state = {
  ...
  componentDidMount() {
    this.updateUptime();
    if (this.props && this.props.online) this.timer = setInterval(() => this.updateUptime(), 3000);
  }
  ...
  updateUptime() {
    ...
    if (!new Date(String(this.state.startTime)).getTime()) return "N/A";

    let diffMs = new Date().valueOf() - new Date(String(this.state.startTime)).valueOf();
  ...
  render() {
    const { id, name, is_blocked, online, load, ping } = this.props;
    return (
      <>
        <tr>
...
interface JudgeStatusesProps {
  [key: string]: unknown;
}

interface JudgeStatusesState {
  judges: Judge[];
  currPage: number;
  pageCount: number;
  count: number;
...
class JudgeStatuses extends React.Component<JudgeStatusesProps, JudgeStatusesState> {
  constructor(props: JudgeStatusesProps) {
    super(props);
    this.state = {
      judges: [],
      currPage: 0,
      ...
      <>
        <div className="judge-table wrapper-vanilla">
          ...
          <div className="judge-table text-left">
            ...
            <ul className="m-1">
              {avaiRuntime.map((runtime, idx) => (
                <li key={idx}>{runtime}</li>
              ))}
            </ul>
          </div>
        </div>
