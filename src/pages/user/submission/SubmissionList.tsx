...
const verdictExplains: Record<string, string> = {
...
function getVerdict(ver: string): string {
...
class FilterPlus extends React.Component<FilterPlusProps> {
  render() {
    return (
      <Link
...
interface SubListItemProps {
  id: number;
  problem: { shortname: string; title: string; points: number };
  user: string;
  status: string;
  result?: string;
  time: number;
  ...
  points?: number;
  language: string;
  contest_object?: string;
  is_frozen?: boolean;
  virtual?: string;
  yourUsername: string;
  contest?: Record<string, unknown>;
  setFilterParams: (key: string, value: string) => void;
  rowid?: number;
}

class SubListItem extends React.Component<SubListItemProps> {
  ...
  render() {
    ...
    const linkPrefix = this.props.contest ? `/contest/${String(this.props.contest.key)}` : "";

    const verdict = status === "D" ? result || status : status;
          ...
          <div className="verdict-container">
            ...
            {typeof points === "number" ? (
              ...
              <div className="points-container available">
                <span className="sub-points text-truncate">{is_frozen ? "?" : "n/a"}</span>
                <span className="prob-points text-truncate">{max_points}</span>
              </div>
            )}
          </div>
...
interface SubmissionListState {
  ...
  errors: unknown;
  isFiltered: boolean;
  refetchDisable: boolean;
  rejudgeDisable: boolean;
}
...
class SubmissionList extends React.Component<SubmissionListProps, SubmissionListState> {
  ...
  callApi(params = { page: 0 }) {
    ...
    if (this.isInContest()) {
      ...
      let prms: Record<string, unknown> = { page: params.page + 1, ...extraParams };
  ...
  componentDidUpdate(prevProps: SubmissionListProps, _prevState: SubmissionListState) {
  ...
  onRefetchClick() {
    ...
    this.setState({ refetchDisable: true }, () => {
      this.callApi();
      setTimeout(() => this.setState({ refetchDisable: false }), values.refetchDisableDuration);
    });
  }
  ...
  onMassRejudgeClick() {
    ...
    if (this.isInContest()) {
      ...
      let prms: Record<string, unknown> = { ...filterParams };
                ...
                <>
                  {this.state.count > 0 ? (
                    this.state.submissions.map((sub, idx) => (
                      <SubListItem
                        key={`sub-${(sub as { id: number }).id}`}
                        contest={this.context.contest as Record<string, unknown>}
                        rowid={idx}
                        {...(sub as SubListItemProps)}
...
const mapStateToProps = (state: {
  user: { user: { username?: string; is_staff?: boolean } | null };
